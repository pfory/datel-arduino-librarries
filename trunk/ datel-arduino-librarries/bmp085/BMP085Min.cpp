...
void BMP085Min::init(int32_t _initVal){     
  getCalData();               // initialize cal data
  calcTrueTemperature();      // initialize b5
  setLocalAbsAlt(_initVal);
}
/*
byte BMP085Min::getDevAddr() {   
  return _dev_address;
}

byte BMP085Min::getMode(){
  return _oss;
}

void BMP085Min::setMode(byte _BMPMode){
  _oss = _BMPMode;
}
*/

/*void BMP085Min::setLocalPressure(int32_t _Pa){   
  int32_t tmp_alt;
...
*/
void BMP085Min::setLocalAbsAlt(int32_t _centimeters){  
  int32_t tmp_Pa;
...
/*void BMP085Min::setAltOffset(int32_t _centimeters){
  _cm_Offset = _centimeters;
}

void BMP085Min::sethPaOffset(int32_t _Pa){
  _Pa_Offset = _Pa;
}
*/
/*void BMP085Min::zeroCal(int32_t _Pa, int32_t _centimeters){
  setAltOffset(_centimeters - _param_centimeters);    
  sethPaOffset(_Pa - _param_datum);    
}
*/

void BMP085Min::getPressure(int32_t *_Pa){   
  long TruePressure;
...
/*
void BMP085Min::getAltitude(int32_t *_centimeters){
  long TruePressure;
...
*/
/*void BMP085Min::getTemperature(int32_t *_Temperature) {
  calcTrueTemperature();                            // force b5 update
  *_Temperature = ((b5 + 8) >> 4);
}
*/
void BMP085Min::calcTrueTemperature(){
  long ut,x1,x2;
...
void BMP085Min::calcTruePressure(long *_TruePressure) {
 ...
 //read Raw Pressure
  writemem(CONTROL, READ_PRESSURE+(MODE_ULTRA_HIGHRES << 6));
  delay(26);    
  readmem(CONTROL_OUTPUT, 3, _buff);  
  up = ((((long)_buff[0] <<16) | ((long)_buff[1] <<8) | ((long)_buff[2])) >> (8-MODE_ULTRA_HIGHRES)); // uncompensated pressure value
...
void BMP085Min::writemem(uint8_t _addr, uint8_t _val) {
...
void BMP085Min::readmem(uint8_t _addr, uint8_t _nbytes, uint8_t __buff[]) {
  Wire.beginTransmission(_dev_address); // start transmission to device 
